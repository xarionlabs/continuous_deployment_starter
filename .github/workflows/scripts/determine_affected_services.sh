...
if [ -d "services" ]; then
  ...
  echo "Warning: 'services' directory not found. No services to process."
  echo ""
  exit 0
fi

if [ ${#ALL_SERVICES_ARRAY[@]} -eq 0 ]; then
    echo "No services found in the 'services' directory."
    echo ""
    exit 0
fi

echo "All known services: ${ALL_SERVICES_ARRAY[@]}"
echo "Changed files input: '$CHANGED_FILES_INPUT'"
echo "Assume potential value changes flag: $ASSUME_POTENTIAL_VALUE_CHANGES"
  ...
  for file in "${CHANGED_FILES[@]}"; do
    ...
    if [[ "$file" == services/* ]]; then
      ...
      if [[ ! " ${AFFECTED_SERVICES_ARRAY[@]} " =~ " ${service_name} " ]]; then
        echo "Service '$service_name' affected by change in '$file'."
    ...
    # Removed generate_quadlet_of_service.sh from this list. Changes to it will apply to services as they are processed.
      ...
      echo "Core Quadlet generation script '$file' changed. Flagging all services for update."
      ...
      echo "Environment/secret script '$file' changed. Flagging all services for update."
      FORCE_UPDATE_ALL=true
      break
    elif [[ "$file" == .github/workflows/release.yml ]]; then
      echo "Release workflow file '$file' changed. Flagging all services for update (as a precaution)."
      FORCE_UPDATE_ALL=true
      break
    fi
  done
else
  echo "No files reported as changed."
fi

if $FORCE_UPDATE_ALL; then
  echo "FORCE_UPDATE_ALL is true (due to critical script change or explicit flag). All services will be updated."
  AFFECTED_SERVICES_OUTPUT="${ALL_SERVICES_ARRAY[@]}"
elif [ ${#AFFECTED_SERVICES_ARRAY[@]} -eq 0 ] && [ "$ASSUME_POTENTIAL_VALUE_CHANGES" == "true" ]; then
  echo "No specific code changes detected, but ASSUME_POTENTIAL_VALUE_CHANGES is true. Flagging all services for update."
  AFFECTED_SERVICES_OUTPUT="${ALL_SERVICES_ARRAY[@]}"
  ...
  echo "No specific services affected by file changes, and ASSUME_POTENTIAL_VALUE_CHANGES is false."
  AFFECTED_SERVICES_OUTPUT=""
else
  echo "Specific services affected by file changes: ${AFFECTED_SERVICES_ARRAY[@]}"
  AFFECTED_SERVICES_OUTPUT="${AFFECTED_SERVICES_ARRAY[@]}"
fi

echo "Final affected services string: '$AFFECTED_SERVICES_OUTPUT'"
