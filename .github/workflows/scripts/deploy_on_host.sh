...
# --- Parameters (passed from GitHub Actions workflow) ---
AFFECTED_SERVICES="${1:-}" # Space-separated string of service names
VARS_JSON_STR="${2:-{\}}"   # JSON string of GitHub Actions vars (defaults to empty JSON object)
SECRETS_JSON_STR="${3:-{\}}" # JSON string of GitHub Actions secrets (defaults to empty JSON object)
...
SERVICES_DEF_DIR_ON_HOST_DEFAULT="$PROJECT_DIR_ON_HOST_DEFAULT/services"
HOST_SCRIPTS_DIR_DEFAULT="$PROJECT_DIR_ON_HOST_DEFAULT/.github/workflows/scripts"
...
HOST_SCRIPTS_DIR="${TEST_OVERRIDE_HOST_SCRIPTS_DIR:-$HOST_SCRIPTS_DIR_DEFAULT}"
...
RELEASE_TOOL_IMAGE_REPO_NAME="continuous_deployment_starter" # Assuming this is the repo name
RELEASE_TOOL_IMAGE_NAME="release-tool"
RELEASE_TOOL_IMAGE_TAG="latest"
...
log_info "VARS_JSON_STR received: $(if [ -z "$VARS_JSON_STR" ] || [ "$VARS_JSON_STR" == "{}" ]; then echo "false"; else echo "true"; fi)"
log_info "SECRETS_JSON_STR received: $(if [ -z "$SECRETS_JSON_STR" ] || [ "$SECRETS_JSON_STR" == "{}" ]; then echo "false"; else echo "true"; fi)"
...
if [ -f "$HOST_SCRIPTS_DIR/refresh_podman_secrets.sh" ]; then
    "$HOST_SCRIPTS_DIR/refresh_podman_secrets.sh" "$SECRETS_JSON_STR"
else
    log_warning "refresh_podman_secrets.sh not found at $HOST_SCRIPTS_DIR. Skipping."
fi


log_step "Creating/Updating Environment Variables File (Host Script)"
if [ -f "$HOST_SCRIPTS_DIR/create_env_variables.sh" ]; then
    ...
    (cd "$PROJECT_DIR_ON_HOST" && "$HOST_SCRIPTS_DIR/create_env_variables.sh")
else
    log_warning "create_env_variables.sh not found at $HOST_SCRIPTS_DIR. Skipping."
fi
...
if [ -f "$HOST_SCRIPTS_DIR/check-service-envs.sh" ]; then
    "$HOST_SCRIPTS_DIR/check-service-envs.sh"
else
    log_warning "check-service-envs.sh not found at $HOST_SCRIPTS_DIR. Skipping."
fi
...
if [ -f "$HOST_SCRIPTS_DIR/generate_meta_services.sh" ]; then
    "$HOST_SCRIPTS_DIR/generate_meta_services.sh"
else
    log_warning "generate_meta_services.sh not found at $HOST_SCRIPTS_DIR. Skipping."
fi
